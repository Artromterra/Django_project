...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs) -> None:
        ...
        if not fixtures_list:
            self.stdout.write(self.style.WARNING(f'The folder "{FIXTURE_DIR}" is empty.'))
        else:
            while fixtures_list:
                err_list.clear()
                for item in fixtures_list:
                    ...
                    try:
                        management.call_command('loaddata', os.path.normpath(os.path.join(FIXTURE_DIR, item)))
    ...
    def _get_list_fixtures(self) -> List:
        path_fixtures = os.path.normpath(os.path.abspath(FIXTURE_DIR))
